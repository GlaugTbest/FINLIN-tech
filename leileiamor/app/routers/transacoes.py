...
def _normalizar_tipo(tipo: str) -> str:
    """
    Normaliza o tipo de transação para 'receita' ou 'despesa'.
    Aceita também 'entrada'/'saida' vindos do frontend.
    """
    tipo_normalizado = (tipo or "").lower()
    if tipo_normalizado in {"receita", "entrada"}:
        return "receita"
    if tipo_normalizado in {"despesa", "saida"}:
        return "despesa"
    return ""
...
def list_transacoes(
    ...
    if tipo:
        tipo_norm = _normalizar_tipo(tipo) or tipo
        query = query.filter(Transacao.tipo == tipo_norm)
    if id_conta:
...
def create_transacao(
    ...
    tipo_normalizado = _normalizar_tipo(transacao_data.tipo)
    if not tipo_normalizado:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Tipo de transacao invalido. Use 'receita' ou 'despesa'."
        )

    # Valida se tipo da transacao corresponde ao tipo da categoria
    if tipo_normalizado != categoria.tipo:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Tipo da transacao ({tipo_normalizado}) nao corresponde ao "
                f"tipo da categoria ({categoria.tipo})"
            ),
        )
...
def update_transacao(
    ...
    if "tipo" in update_data:
        tipo_norm = _normalizar_tipo(update_data["tipo"])
        if not tipo_norm:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Tipo de transacao invalido. Use 'receita' ou 'despesa'."
            )
        update_data["tipo"] = tipo_norm
    ...
    if _normalizar_tipo(tipo_antigo) == "receita":
    ...
    if _normalizar_tipo(transacao.tipo) == "receita":
